...
Registry-global calls are also atomic. Each action verifies its target postcondition before emitting success. Range execution can isolate malformed or unauthorized live targets, but operators must reread the live source and perform a final complete `done()` check.
