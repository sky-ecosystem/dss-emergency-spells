...
CAST="$root/test/scripts/mock-cast.sh" FORGE="$root/test/scripts/mock-forge.sh" GIT="$root/test/scripts/mock-git.sh" \
    "$validator" "$manifest" mock:// "$root" "$factory" create "Incident batch" "$leaf2" "$leaf1"
CAST="$root/test/scripts/mock-cast.sh" FORGE="$root/test/scripts/mock-forge.sh" GIT="$root/test/scripts/mock-git.sh" \
    "$validator" "$manifest" mock:// "$root" "$factory" create2 "Incident batch" "$leaf1" "$leaf2"

if CAST="$root/test/scripts/mock-cast.sh" FORGE="$root/test/scripts/mock-forge.sh" GIT="$root/test/scripts/mock-git.sh" \
    "$validator" "$manifest" mock:// "$root" "$factory" create2 "Incident batch" "$leaf2" "$leaf1" >/dev/null 2>&1; then
    echo "expected unordered deterministic leaves to fail preflight" >&2
    exit 1
fi

if CAST="$root/test/scripts/mock-cast.sh" FORGE="$root/test/scripts/mock-forge.sh" GIT="$root/test/scripts/mock-git.sh" \
    "$validator" "$manifest" mock:// "$root" "$factory" create "Incident batch" "$global" >/dev/null 2>&1; then
    echo "expected registry-global record to fail leaf preflight" >&2
    exit 1
fi

if CAST="$root/test/scripts/mock-cast.sh" FORGE="$root/test/scripts/mock-forge.sh" GIT="$root/test/scripts/mock-git.sh" \
    "$validator" "$manifest" mock:// "$root" "$factory" create "" "$leaf1" >/dev/null 2>&1; then
