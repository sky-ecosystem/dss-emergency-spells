...
Deployment validation uses this repository's `src/` tree. Git verifies that the repository is clean, checked out at the exact signed `sourceCommit` recorded for the artifact, and Foundry forces a fresh compilation before comparing deployment bytecode.
